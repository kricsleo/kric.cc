...
(2022/07/17 15:14 现在已经抛弃了现有工具, 从零使用 Rust 编译出整个静态站点)
...
```bash
# es
docker run -d --name es --net host --restart unless-stopped kricsleo/elasticsearch-ik

# nginx
docker run -d --name nginx --net host --restart unless-stopped ranadeeppolavarapu/nginx-http3

# blog-server
docker run -d --name blog-server --net host --restart unless-stopped kricsleo/blog-server
```
